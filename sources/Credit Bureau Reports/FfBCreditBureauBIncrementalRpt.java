...
/*-----------------------------------------------------------------------------
* @author Harshini Sakthivel /Deepakumar
* Date Created: 23-01-2025 
* Attached as : Verification job in BATCH>BNK/FF.B.CBR.INTERIM.REPORT
* EB.API>FF.B.CBR.INTERIM.REPORT
* PGM.FILE>FF.B.CBR.INTERIM.REPORT
* BATCH>MFI/FF.B.CBR.INTERIM.REPORT
* BATCH>BNK/FF.B.CBR.INTERIM.REPORT
* TSA.SERVICE>BNK/FF.B.CBR.INTERIM.REPORT
* TSA.SERVICE>MFI/FF.B.CBR.INTERIM.REPORT
* EB.FF.PARAMETER>FF.CBR.REPORT.PATH        --->For path
* 
* Description: Extract the CRB Report based on the Required field mapping
*-----------------------------------------------------------------------------*/
public class FfBCreditBureauBIncrementalRpt extends ServiceLifecycle {
    DataAccess da = new DataAccess(this);
    Session ss = new Session(this);
    String todayDate = ss.getCurrentVariable("!TODAY");
    Set<String> finalArrId = new HashSet<>();
    List<String> prefinalArrId = new ArrayList<>();
    ...
    AaActivityHistoryRecord aaActHistRec1 = null;
    ...
    public static final String AAACTIVITYHISTORY = "AA.ACTIVITY.HISTORY";
    public static final String LENDINGAPPLYPAYMENTPRCOLLECTION = "LENDING-APPLYPAYMENT-PR.COLLECTION";
    public static final String EBFFLOANDPD = "EB.FF.LOAN.DPD";
    ...
    private static final int[] FIELD_LENGTHS = { 6, 35, 30, 30, 20, 100, 50, 50, 30, 8, 3, 8, 1, 3, 100, 3, 100, 3, 100,
            ...
            8, 8, 8, 9, 9, 9, 3, 3, 9, 9, 9, 3, 9, 8, 20, 3, 3, 1, 3, 10, 3, 5, 30, 1 };
    ...
    public List<String> getIds(ServiceData serviceData, List<String> controlList) {
        try {
            ...
            cBReportId = todayDate + "-INTERIM";
            EbFfCreditBureauReportRecord ffCBRec = new EbFfCreditBureauReportRecord(
                    da.getRecord(finMnemonic, "EB.FF.CREDIT.BUREAU.REPORT", "", cBReportId));
            String cbrGen = ffCBRec.getCbrGeneration().getValue();
            if (cbrGen.equalsIgnoreCase("RANGE")) {
                fromDate = ffCBRec.getFromDate().getValue();
                toDate = ffCBRec.getToDate().getValue();
            }
            boolean isDaily = fromDate.equals(toDate);
            String dateCondition = "";
            if (isDaily) {
                dateCondition = " EQ " + fromDate;
            } else {
                dateCondition = " GE " + fromDate + " AND LE " + toDate;
            }
            getDateCondition(dateCondition);
            List<String> aaArrList = da.selectRecords(finMnemonic, AAARRANGEMENT, "",
                    "WITH ARR.STATUS EQ CURRENT OR ARR.STATUS EQ EXPIRED OR ARR.STATUS EQ PENDING.CLOSURE");
            for (int i = 0; i < aaArrList.size(); i++) {
                aaActHisRec = new AaActivityHistoryRecord(
                        da.getRecord(finMnemonic, AAACTIVITYHISTORY, "", aaArrList.get(i)));
                List<EffectiveDateClass> effectiveDateList = aaActHisRec.getEffectiveDate();
                getEffectiveDateClass(aaArrList, i, effectiveDateList);
            }
            List<String> ebFfLoanDpdSel = da.selectRecords(finMnemonic, EBFFLOANDPD, "", "WITH DATE" + dateCondition);
            for (String ebFfLoanDpd : ebFfLoanDpdSel) {
                finalArrId.add(ebFfLoanDpd.split("-")[0]);
            }
            List<String> arrIdClosedSel = da.selectRecords(finMnemonic, AAARRANGEMENT, "",
                    "WITH ARR.STATUS EQ CLOSE AND CLOSED.DATE" + dateCondition);

            finalArrId.addAll(arrIdClosedSel);

            finalArrList.addAll(finalArrId);
        } catch (Exception e) {
            e.getMessage();
        }
        return finalArrList;
    ...
    private void getEffectiveDateClass(List<String> aaArrList, int i, List<EffectiveDateClass> effectiveDateList) {
        try {
            for (EffectiveDateClass effectiveDate : effectiveDateList) {
                ...
                for (ActivityRefClass activityRef : activityRefList) {
                    ...
                    if (activity.equals(LENDINGAPPLYPAYMENTPRCOLLECTION)) {
                        String activeSystemDate = activityRef.getSystemDate().getValue();
                        LocalDate systemDate = LocalDate.parse(activeSystemDate, formatter);
                        LocalDate ebCbrfromDate = LocalDate.parse(fromDate, formatter);
                        LocalDate ebCbrtodayDate = LocalDate.parse(toDate, formatter);
                        if ((!systemDate.isBefore(ebCbrfromDate)) && (!systemDate.isAfter(ebCbrtodayDate))) {
                            finalArrId.add(aaArrList.get(i));
                        }
                    }
                }
            }
    ...
    private void getDateCondition(String dateCondition) {
        try {
            List<String> arrIdList = da.selectRecords(finMnemonic, AAARRANGEMENT, "",
                    "WITH ARR.STATUS EQ CURRENT OR ARR.STATUS EQ PENDING.CLOSURE OR ARR.STATUS EQ EXPIRED AND START.DATE"
                            + dateCondition);
            finalArrId.addAll(arrIdList);
            List<String> migArrIdList = da.selectRecords(finMnemonic, AAARRANGEMENT, "",
                    "WITH ARR.STATUS EQ CURRENT OR ARR.STATUS EQ PENDING.CLOSURE OR ARR.STATUS EQ EXPIRED AND ORIG.CONTRACT.DATE"
                            + dateCondition);
            finalArrId.addAll(migArrIdList);
        } catch (Exception e) {
            e.getMessage();
        }
    }
        ...
        try {
            ...
            if ((arrStatus.equals(CURRENT)) || (arrStatus.equals("EXPIRED"))) {
                ...
                if (arrStatus.equals("CLOSE")) {
                    ...
                    arrDateClosed = "";
                    getAaActivityHistoryRecord(arrId);
                }
                accountStatus = "S07";
                LocalDate closedDate = LocalDate.parse(arrDateClosed, formatter);
                LocalDate today = LocalDate.parse(toDate, formatter);
                LocalDate fromdate1 = LocalDate.parse(fromDate, formatter);

                if ((!closedDate.isBefore(fromdate1)) && (!closedDate.isAfter(today))) {
    ...
    private void getAaActivityHistoryRec() {
        try {
    ...
    private void getAaActivityHistoryRecord(String arrId) {
        try {
            aaActHistRec1 = new AaActivityHistoryRecord(da.getRecord(finMnemonic, AAACTIVITYHISTORY, "", arrId));
            List<EffectiveDateClass> effectiveDateList = aaActHistRec1.getEffectiveDate();
            for (EffectiveDateClass effectiveDate : effectiveDateList) {
                ...
                for (ActivityRefClass activeRef : activeRefList) {
                    ...
                    if ((activity.equals("LENDING-WRITE.OFF-BAL.MAINTAIN"))
                            ...
                            || (activity.contains(LENDINGAPPLYPAYMENTPRCOLLECTION))) {
    ...
    private void getDateOfLastPayment(String migratedContractFlg) {
        try {
            if ("YES".equalsIgnoreCase(migratedContractFlg)) {
                ...
                getCreditValueDate(aaActHistRec1);
            }
        } catch (Exception e) {
            e.getMessage();
        }
    ...
    private void getCreditValueDate(AaActivityHistoryRecord aaActHistRec1) {
        try {
            List<EffectiveDateClass> effectiveDateList1 = aaActHistRec1.getEffectiveDate();
    ...
    private void generatingTheCBRFile(AaArrangementRecord aaArrRec, String arrId, Contract contract,
        ...
        try {
            ...
            String outputPath = paramPath + "temp" + "_" + "CBRIncrementalSubmissionsReportExtract" + "_" + finMnemonic
                    + "_" + ss.getSessionNumber() + ".cdf";
    ...
    public void getEbFfLoanDPDetails(String arrId) {
        try {
            ...
            EbFfLoanDpdRecord loanDpdRec = new EbFfLoanDpdRecord(da.getRecord(finMnemonic, EBFFLOANDPD, "", dpdRecId));
    ...
    private void getFamilyIncDet(EbFfLoanDetailsRecord ebLoanDetRec) {
        try {
            ...
            if (fmEntityNumList == null || fmEntityNumList.isEmpty()) {
                return;
            }
            for (FmEntityNumberClass fmEntityNum : fmEntityNumList) {
                boolean isEligible = fmEntityNum != null && fmEntityNum.getFmEntityNumber() != null
                        && fmEntityNum.getFmEntityNumber().getValue() != null
                        && fmEntityNum.getIsEligibleHouseholdMember() != null
                        && "YES".equalsIgnoreCase(fmEntityNum.getIsEligibleHouseholdMember().getValue());
                if (!isEligible) {
                    continue;
                }
                String fmEntNum = fmEntityNum.getFmEntityNumber().getValue().trim();
                totalMonthlyIncome += calculateIncome(fmEntNum, incomeList);
                totalMonthlyExpense += calculateExpense(fmEntNum, expenseList);
            }
    ...
    private double calculateIncome(String fmEntNum, List<InEntityNumberClass> incomeList) {

        double totalIncome = 0.0;

        if (incomeList == null || fmEntNum == null) {
            return 0.0;
        }

        for (InEntityNumberClass incomeRecord : incomeList) {

            boolean isValid = incomeRecord != null && incomeRecord.getInEntityNumber() != null
                    && incomeRecord.getInEntityNumber().getValue() != null
                    && fmEntNum.equals(incomeRecord.getInEntityNumber().getValue().trim());

            if (isValid) {

                totalIncome += convertToMonthly(
                        incomeRecord.getIncomePerPeriod() != null ? incomeRecord.getIncomePerPeriod().getValue() : null,
    ...
    private double calculateExpense(String fmEntNum, List<ExEntityNuberClass> expenseList) {

        double totalExpense = 0.0;

        if (expenseList == null || fmEntNum == null) {
            return 0.0;
        }

        for (ExEntityNuberClass expenseRecord : expenseList) {

            boolean isValid = expenseRecord != null && expenseRecord.getExEntityNuber() != null
                    && expenseRecord.getExEntityNuber().getValue() != null
                    && fmEntNum.equals(expenseRecord.getExEntityNuber().getValue().trim());

            if (isValid) {

                totalExpense += convertToMonthly(
    ...
    private double convertToMonthly(String amountStr, String frequency) {
