...
* EB.API>FF.B.CB.MONT.CANCEL.REPORT
* EB.API>FF.B.CB.MONT.CANCEL.REPORT.SELECT
* PGM.FILE>FF.B.CB.MONT.CANCEL.REPORT
* BATCH>MFI/FF.CREDIT.BUREAU.REPORT 
* TSA.SERVICE>BNK/FF.CREDIT.BUREAU.REPORT
* TSA.SERVICE>MFI/FF.CREDIT.BUREAU.REPORT
* TSA.SERVICES.COB>FF.CREDIT.BUREAU.REPORT
* EB.FF.PARAMETER>FF.CBR.REPORT.PATH        --->For path
* 
* Description: Extract the CRB Month End Cancel Report based on the Required field mapping
*-----------------------------------------------------------------------------*/
public class FfBCreditBurMontEndCancelReport extends ServiceLifecycle {
    ...
    public void process(String id, ServiceData serviceData, String controlItem) {
        ...
        try {
            ...
            if (cancelAccountNo == null || cancelAccountNo.trim().isEmpty()) {
                return;
            }
            getAccountRecord(cancelAccountNo);
            reportedDate = determineReportedDate(todayDate);
            ...
            if (cancelOpeningDate.trim().isEmpty()) {
                return;
            }
            ...
            if ((aaArrStartDate.isBefore(startDate)) || (aaArrStartDate.equals(lastDateOfMonth))) {
    ...
    private boolean getAccountRecord(String cancelAccountNo) {
        try {
            if (cancelAccountNo == null || cancelAccountNo.trim().isEmpty()) {
                return false;
            }
            acNum = cancelAccountNo.trim();
    ...
    private String determineReportedDate(String dateFromRecord) {
        try {
            DateTimeFormatter inputFormatter = dateFromRecord.matches("\\d{8}") ? DateTimeFormatter.ofPattern(YYYYMMDD)
            ...
            if (day > lastDay - 4) {
                ...
                return prevMonthEnd.format(DateTimeFormatter.ofPattern(YYYYMMDD));
            }
        } catch (Exception e) {
            e.getMessage();
            return "";
    ...
    private void generatingTheCBRFile(String arrId, Contract contract, ServiceData serviceData) {
        try {
            ...
            String outputPath = paramPath + "temp" + "_" + "CBRReportExtract" + "_" + finMnemonic + "_"
