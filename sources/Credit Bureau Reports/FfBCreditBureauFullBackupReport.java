...
* Attached as : Verification job in BATCH>BNK/FF.CREDIT.BUREAU.FULL.REPORT
* BATCH>MFI/FF.CREDIT.BUREAU.FULL.REPORT
* EB.API>FF.B.CB.FULL.REPORT
* EB.API>FF.B.CB.FULL.REPORT.SELECT
* PGM.FILE>FF.B.CB.FULL.REPORT
* TSA.SERVICE>MFI/FF.CREDIT.BUREAU.FULL.REPORT
* TSA.SERVICE>BNK/FF.CREDIT.BUREAU.FULL.REPORT
...
*-----------------------------------------------------------------------------*/
public class FfBCreditBureauFullBackupReport extends ServiceLifecycle {
    private static final FusionFileLogger fullBackupReportlog = FusionFileLogger
            .getLogger(FfBCreditBureauFullBackupReport.class);
    ...
    private void getAaActivityHistoryRecord() {
        try {
        ...
        try {
            if (contractId.startsWith("FT")) {
                ...
                if ((!ftRec.toString().isEmpty()) && (transacType.toUpperCase().equalsIgnoreCase("ACRP"))) {
                    creditValueTransDate = getSafeValue(ftRec.getCreditValueDate().getValue());
                }
            }
        } catch (Exception e) {
            e.getMessage();
    ...
    private void generatingTheCBRFile(AaArrangementRecord aaArrRec, String arrId, Contract contract,
        ...
        try {
            ...
            getTheDetailsFromCustomerRecord(cusId, contractDate);
            ...
            String outputPath = paramPath + "temp" + "_" + "CBRFullBackupReportExtract" + "_" + finMnemonic + "_"
    ...
    private void getAaActivityHistoryRec(String arrId) {
        try {
    ...
    private void getEbFfCollectionDetsHistoryRecord(String arrId) {
        try {
            String arId = arrId + "-" + strtDate + ".01";
            histRec = new EbFfCollectionDetsHistoryRecord(
                    da.getRecord(finMnemonic, EBFFCOLLECTIONDETSHISTORY, "", arId));
        } catch (Exception e) {
            e.getMessage();
        }
    }
    ...
    private void getTheDetailsFromCustomerRecord(String cusId, String contractDate) {
        ...
        try {
            ...
            cusRec = new CustomerRecord(da.getRecord(mnemonic, "CUSTOMER", "", cusId));
            getCustomerName(cusRec);
            if (cusRec.getDateOfBirth() != null) {
                cusDateOfBirth = cusRec.getDateOfBirth().getValue();
            } else {
                cusDateOfBirth = "";
            }
            ...
            getCusDateOfBirth(contractDate, cusRec);
    ...
    private void getCusDateOfBirth(String contractDate, CustomerRecord cusRec) {
        try {
    ...
    private void getAaArrAccount(Contract contract, String originalContractDate) {
        fullBackupReportlog.info("getAaArrAccount method start " + originalContractDate + "****arrId" + arrId);
        try {
            ...
            for (String aaArrAcctid : aaArrAccountPrptyList) {
                fullBackupReportlog.info("For Loop-2017");
