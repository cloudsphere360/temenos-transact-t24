...
* Attached as : Verification job in BATCH>BNK/FF.B.CBR.LOAN.CORRECTION.REPORT
* EB.API>FF.B.CBR.LOAN.CORRECTION
* PGM.FILE>FF.B.CBR.LOAN.CORRECTION
* BATCH>MFI/FF.B.CBR.LOAN.CORRECTION
* BATCH>BNK/FF.B.CBR.LOAN.CORRECTION
* TSA.SERVICE>BNK/FF.B.CBR.LOAN.CORRECTION.REPORT
* TSA.SERVICE>MFI/FF.B.CBR.LOAN.CORRECTION.REPORT
* EB.FF.PARAMETER>FF.CBR.REPORT.PATH        --->For path
* 
* Description: Extract the CRB Report based on the Required field mapping
*-----------------------------------------------------------------------------*/
public class FfBCreditBureauCorrectionReport extends ServiceLifecycle {
    ...
    public List<String> getIds(ServiceData serviceData, List<String> controlList) {
        try {
            ...
            cbReportId = todayDate + "-LOANS";
            getCBCorrectionRec();
            String cbrGen = ffCBRec.getCbrGeneration().getValue();
            if (cbrGen.equalsIgnoreCase("LOANS")) {
                List<TField> loanIds = ffCBRec.getCbrLoan();
                for (TField loanId : loanIds) {
                    arrId = loanId.getValue();
                    arrList.add(arrId);
                }
            }
        } catch (Exception e) {
    ...
    private void getCBCorrectionRec() {
        try {
            ffCBRec = new EbFfCreditBureauReportRecord(
    ...
    private void getAaActivityHistoryRecord(String arrId) {
        ...
        try {
            AaActivityHistoryRecord aaActHistRec1 = new AaActivityHistoryRecord(
    ...
    private void generatingTheCBRFile(AaArrangementRecord aaArrRec, String arrId, Contract contract,
        ...
        try {
            ...
            String outputPath = paramPath + "temp" + "_" + "CBRLoanCorrectionReport" + "_" + finMnemonic + "_"
