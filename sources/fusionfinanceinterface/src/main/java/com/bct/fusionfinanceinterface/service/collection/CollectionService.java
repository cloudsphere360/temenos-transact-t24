...
public class CollectionService {
	...
	@Value("${fusion.loan.collection.creation.post.url}")
	private String createCollectionPostURL;
	
	public CollectionService(WebClient webClient) {
        this.webClient = webClient;
    }
    public Mono<TransactionData> createCollection(TransactionData data) {
        return webClient.post()
        		  .uri(uriBuilder -> uriBuilder
                          .path(createCollectionPostURL)
                ...
                .bodyValue(data.getOrchestrationRequest().getCollectionDetails())
                .exchangeToMono(response -> processCreateCollectionResponse(response, data))
	...
	private Mono<TransactionData> handleCreateCollectionResponse(String t24Response, TransactionData data) {
	...
	private Mono<TransactionData> processCreateCollectionResponse(ClientResponse response, TransactionData data) {
		HttpStatus status = response.statusCode();

		if (status.is2xxSuccessful()) {
			return response.bodyToMono(String.class).flatMap(body -> handleCreateCollectionResponse(body, data));
		} else {
			return response.bodyToMono(String.class).defaultIfEmpty("").flatMap(body -> {
				...
				return handleCreateCollectionResponse(body, data);
			});
		}
	}
