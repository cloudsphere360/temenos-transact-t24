...
public class LoanOrchestrationService {
    private static final Logger logger = LogManager.getLogger(LoanOrchestrationService.class);
	...
	public LoanOrchestrationService(CustomerService customerService, LoanSimulationService loanSimulationService,
	...
	public TransactionData orchestrateCustomerAccountLoan(TransactionData transactionData) {
		...
		return customerService.createCustomer(transactionData)
				...
				flatMap(result -> {
					logger.info("Step 4: Get Customer Loan Repayment Schedule Details...");
					if ((result.getOrchestrationResponse().getStatus() != null
							&& result.getOrchestrationResponse().getStatus().equals(CommonConstants._SUCCESS))
	...
	public TransactionData orchestrateUpdateLoanSimulation(TransactionData transactionData) {
		logger.info("Starting orchestration for customer ID :: ");
		...
		return loanSimulationService.updateLoanSimulation(transactionData)
				...
				flatMap(result -> {
					logger.info("Step 3: Get Customer Loan Repayment Schedule Details...");
					if ((result.getOrchestrationResponse().getStatus() != null
							&& result.getOrchestrationResponse().getStatus().equals(CommonConstants._SUCCESS))
							&& (result.getOrchestrationResponse().getLoanSimulationId() != null
									&& !result.getOrchestrationResponse().getLoanSimulationId().trim().isEmpty())) {
	...
	public TransactionData orchestrateLoanActivation(TransactionData transactionData) {
