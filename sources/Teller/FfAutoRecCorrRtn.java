...
public class FfAutoRecCorrRtn extends RecordLifecycle {

    private static final FusionFileLogger FfTellerDefaultRout = FusionFileLogger.getLogger(FfAutoRecCorrRtn.class);
    ...
    public void defaultFieldValues(String application, String currentRecordId, TStructure currentRecord,
        ...
        try {
            ...
            id = currentRecordId;
            String[] yCurRecId = id.split("-", 0);
            String yCompId = yCurRecId[0];
            String yTodayDt = yCurRecId[1];

            FfTellerDefaultRout.info(" yCompId -> " + yCompId);
            FfTellerDefaultRout.info(" yTodayDt -> " + yTodayDt);

            LocalDate yTodayDtL = null;
            LocalDate yYestrdayDtL = null;

            yTodayDtL = LocalDate.parse(yTodayDt, formatter);
            yYestrdayDtL = yTodayDtL.minusDays(1);
            String yYtrdayDate = yYestrdayDtL.toString().replace("-", "");
            yestId = yCompId + "-" + yYtrdayDate;
            FfTellerDefaultRout.info("yestId " + yestId);

            /*
             * try { CompanyRecord yCompRec = new CompanyRecord(da.getRecord("COMPANY",
             * yCompId)); fMnemonic = yCompRec.getFinanFinanMne().getValue();
             * FfTellerDefaultRout.info("fMnemonic " + fMnemonic); } catch (Exception e) {
             * e.getMessage(); }
             */
            fMnemonic = ses.getCompanyRecord().getFinancialMne().toString();
            FfTellerDefaultRout.info("fMnemonic " + fMnemonic);
            ...
            try {
                ...
                FfTellerDefaultRout.info("Yesterday's Collection Employees List " + yestRecList.toString());
                int size = yestRecList.size();

                for (int i = 0; i < size; i++) {
                    EmployeeIdClass yestRecObj = yestRecList.get(i);
                    FfTellerDefaultRout.info("yesterday record obj " + yestRecObj.toString());
                    ...
                    FfTellerDefaultRout.info(
                            "Calling updateTodaysRec for Employee = " + empIdYest + " Pending = " + prevPendingYest);

                    updateTodaysRec(collposting, empIdYest, prevPendingYest, id);

                    FfTellerDefaultRout.info("Returned from updateTodaysRec for Employee = " + empIdYest);

                }
            } catch (Exception e) {
                FfTellerDefaultRout.info("Exception while processing yesterday record : " + e.getClass().getName()
                        + " : " + e.getMessage());
            }
            FfTellerDefaultRout.info("collposting Record after Prev pending update " + collposting.toString());
            try {
                FfTellerDefaultRout.info("Entering try block of concat entries");
                rodets = new EbFfEodRoDetsRecord(da.getRecord(fMnemonic, "EB.FF.EOD.RO.DETS", "", id));
                FfTellerDefaultRout.info("Eb Ro Dets record: " + rodets.toString());
